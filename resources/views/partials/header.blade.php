...
<header class="flex flex-col md:flex-row md:items-center md:justify-between border-b border-cyber-border/80 pb-6 gap-4">
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-2xl font-bold tracking-tight text-white flex items-center gap-2.5">
                @cw('title')
                <span class="text-xs font-mono px-2 py-0.5 border border-cyber-green/30 bg-cyber-green/10 text-cyber-green rounded-full shadow-neon-green/10" x-text="'v' + (config.version || '2.1')">v2.1</span>
            </h1>
            <p class="text-sm text-gray-400">@cw('subtitle')</p>
        </div>
    </div>
    ...
    <div class="flex flex-wrap items-center gap-3">
        ...
        <div class="flex items-center gap-2 px-3 py-1.5 bg-cyber-card border border-cyber-border rounded-lg text-xs">
            ...
            <span class="text-gray-300" x-text="polling ? config.labels.polling_active : config.labels.polling_suspended"></span>
        </div>

        <!-- Last Uptime Info -->
        <div class="px-3.5 py-1.5 bg-cyber-card border border-cyber-border rounded-lg text-xs">
            <span class="text-gray-400 font-mono" x-text="config.labels.uptime + ':'"></span>
            <span class="text-cyber-blue font-semibold font-mono ml-1" x-text="metrics.uptime">@cw('loading')</span>
        </div>
